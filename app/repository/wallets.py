...
def add_expense(db: Session, wallet_name: str, user_id: int, amount: Decimal) -> Wallet:
    wallet = db.query(Wallet).filter(Wallet.name == wallet_name, Wallet.user_id == user_id).first()
    wallet.balance -= amount
    return wallet
...
def create_wallet(db: Session, wallet_name: str, amount: float, user_id: int, currency: CurrencyEnum) -> Wallet:
