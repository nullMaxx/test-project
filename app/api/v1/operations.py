...
def get_operation_list(wallet_id: int | None = Query(None), date_from: datetime | None = Query(None), date_to: datetime | None = Query(None), user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def create_transfer(payload: TransferCreateSchema, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    return operations_service.transfer_between_wallets(
        db, user, payload.from_wallet_id, payload.to_wallet_id, payload.amount
    )